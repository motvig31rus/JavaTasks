...
public class AStarState
{
     ...
     **/
    //возвращает клетку с минимальной стоимостью
    public Waypoint getMinOpenWaypoint()
    {
        ...
        //Инициализируем массив, итератор и другое
     ...
     **/
    //
    //добавляет открытую клетку
    //Если она уже была открыта, то сравнивает с уже имеющейся стоимостью:
    //если она меньше, чем новая, то НЕ будет добавлять
    public boolean addOpenWaypoint(Waypoint newWP)
    {
    	// Определяем положение точки
        Location location = newWP.getLocation();
        //Проверяем, есть ли на этом месте открытая точка
        if (openedWaypoints!=null && openedWaypoints.containsKey(location))
        {
        	
            //Если есть, выбираем вэйпоинт с минимальным значением стоимости
            Waypoint current_waypoint = openedWaypoints.get(location);
            if (newWP.getPreviousCost() < current_waypoint.getPreviousCost())
            {
                //Возвращаем true, если заменили на новый вэйпоинт
        ...
        //Если не было открытой точки, то вставляем и возвращаем true
        openedWaypoints.put(location, newWP);
        return true;
    }
    ...
    //вернет количество открытых клеток
